...
- Заиспользовал `CROSS JOIN LATERAL` что позволяет для каждого заказа размножить строки, создав от 1 до 5 позиций в каждом.
...
После этого скрипт заработал и не вешал больше БД. В результате генерации получились следующие результаты:
