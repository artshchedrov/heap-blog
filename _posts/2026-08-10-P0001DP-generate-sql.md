...
В дальнейшем эти скрипты наврятли будут применяться, так как для дальнейших экспериментов понадобиться постоянный поток случайных данных в БД.

### SQL скрипты

Скрипт генерации имеет следующий вид:
...
После гугления и общения с "оракулами", я понял что написал довольно страшный скрипт. При разборе, дело оказалось в том что цикл FOR создает миллионы отдельных вставок, а также рандомная выборка клиента и продукта заставляет сканировать всю таблицу `client` и `product`
...
- Убрал циклы и добавил функцию `generate_series()` для создания строк в памяти, которые потом будут вставлены единой пачкой.
- Заиспользовал `CROSS JOIN LATERAL` что позволяет для каждого заказа размножить строки, создав от 1 до 5 позиций в каждом заказе.
...
После исправлений, скрипт заработал и не вешал больше БД. В результате генерации получились следующие результаты:
